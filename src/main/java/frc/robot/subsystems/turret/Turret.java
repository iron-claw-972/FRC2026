...
public class Turret extends SubsystemBase implements TurretIO {
	...
	public void goToPos(double rads) {
		target = rads;
	}

	public boolean isNearEdge() {
		return (target > Units.degreesToRadians(TurretConstants.MAX_ANGLE - 5))
				|| (target < Units.degreesToRadians(TurretConstants.MIN_ANGLE + 5));
	}
	...
	public void updateInputs() {
		...
		inputs.velocity = Units.rotationsToRadians(motor.getVelocity().getValueAsDouble())
				* TurretConstants.TURRET_RADIUS / GEAR_RATIO;
	}
	...
	public void periodic() {
		...
		// continue;
		// }
		// if (!found || Math.abs(candidate - lastGoalRad) < Math.abs(best -
		// lastGoalRad)) {
		// best = candidate;
		// found = true;
		// }
