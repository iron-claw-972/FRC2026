...
public class AutoShootCommand extends Command {
	...
	public void updateSetpoints(Pose2d drivepose) {
		...
		Logger.recordOutput("current", goalState);
		Translation3d target3d = new Translation3d(target.getX(), target.getY(),
				FieldConstants.getHubTranslation().getZ());
		Translation3d pos3d = new Translation3d(turretPosition.getX(), turretPosition.getY(),
				TurretConstants.DISTANCE_FROM_ROBOT_CENTER.getZ());
		Logger.recordOutput("old/new", ShooterPhysics.getShotParams(
				new Translation2d(
						turretVelocityX, turretVelocityY),
				target3d.minus(pos3d),
				2.0));
